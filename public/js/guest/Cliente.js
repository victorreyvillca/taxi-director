...
com.em.Cliente.prototype = {
	...
	initEvents: function() {with(this) {
		...
		$("#lastnameFilter").bind('keydown', function(e) {
			if (e.type == 'keydown' && e.which == '13') {
				initDisplayStart();
				table.oApi._fnAjaxUpdate(table.fnSettings());
			}
		...
		$('#ciFilter').css('width','15%');
		$("#ciFilter").bind('keydown', function(e) {
			if (e.type == 'keydown' && e.which == '13') {
				initDisplayStart();
				table.oApi._fnAjaxUpdate(table.fnSettings());
			}
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblCliente input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblCliente input[id^=check-slave-]').attr('checked', false);
			}
		});
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"oLanguage": {
				"sEmptyTable": 'No existe Clientes.'
			},
...
//				clickToUpdate('#tblProducto a[id^=update-datoPromocion-]');
			...
			"fnServerData": function (sSource, aoData, fnCallback ) {
				...
				var position = getPosition(aoData, 'nombres');
				if (position == -1)
					aoData.push({'name': 'nombres','value': $('#productoFilter').attr('value')});
				else
					aoData[position].value=$('#productoFilter').attr('value');

				position = getPosition(aoData, 'apellidos');
				if (position == -1)
					aoData.push({'name': 'apellidos','value': $('#lastnameFilter').attr('value')});
				else
					aoData[position].value=$('#lastnameFilter').attr('value');
				
				position = getPosition(aoData, 'ci');
				if (position == -1)
					aoData.push({'name': 'ci','value': $('#ciFilter').attr('value')});
				else
					aoData[position].value=$('#ciFilter').attr('value');
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			event.preventDefault();
			// Serializes items checked
			var items = $('#tblCliente :checked');
			var itemsChecked = items.serialize();
			if (itemsChecked == '') {
				alert.flashInfo('No hay Cliente seleccionado', {header: com.em.Alert.NOTICE});
				return;
			}
			var action = $(this).attr('href');
