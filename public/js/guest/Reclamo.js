...
com.em.Reclamo.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblReclamo input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblReclamo input[id^=check-slave-]').attr('checked', false);
			}
		});
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"oLanguage": {
				"sEmptyTable": 'No existe Reclamos.'
			},
			"fnDrawCallback": function() {
				clickToUpdate('#tblReclamo a[id^=edit-reclamo-]');
			},
...
//							setValidatorForm("#formId");
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			event.preventDefault();
			// Serializes items checked
			var items = $('#tblReclamo :checked');
			var itemsChecked = items.serialize();
			if (itemsChecked == '') {
				alert.flashInfo('No hay Reclamo seleccionado', {header: com.em.Alert.NOTICE});
				return;
			}
			var action = $(this).attr('href');
