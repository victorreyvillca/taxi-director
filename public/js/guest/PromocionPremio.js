...
var com = com || {};
com.em = com.em ||{};
	com.em.PromocionPremio = function () {
...
com.em.PromocionPremio.prototype = {
...
//		oSettings._iDisplayLength = 3;
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblPromocionPremio input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblPromocionPremio input[id^=check-slave-]').attr('checked', false);
			}
		});		
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"fnDrawCallback": function() {
				clickToUpdate('#tblPromocionPremio a[id^=update-promocionPremio-]');
			},
...
//					aoData.push({"name": "filter_name","value": $('#nameFilter').attr('value')});
					aoData.push({"name": "promocionId","value": $('#promocionId').attr('value')});
				} else {
//					aoData[position].value=$('#nameFilter').attr('value');
					aoData[position].value=$('#promocionId').attr('value');
				}
...
//				return '<a id="update-promocionPremio-'+oObj.aData[0]+'" href="'+url.toUpdate+'/id/'+oObj.aData[0]+'">'+oObj.aData[1]+'</a>';
//				}
			}
				);
		columns.push({
			...
			fnRender : function (oObj) {
				if (oObj.aData[2] == 0) {
					return '<input type="text" name="cantidades[]" id="cantidad-'+oObj.aData[0]+'" value="">';
				} else {
					return '<input type="text" name="cantidades[]" id="cantidad-'+oObj.aData[0]+'" value="'+oObj.aData[2]+'">';
				}
			}
		});
		
		//$row[] = '<input type="text" id="cantidad'.$promocionPremio->getId().'" name="cantidad'.$promocionPremio->getId().'">';
...
//		});
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			var items = $('#tblPromocionPremio :checked');
			var itemsChecked = items.serialize();
			if (itemsChecked == '') {
				alert.flashInfo('No hay Promocion Seleccionada', {header: com.em.Alert.NOTICE});
				return;
			}
			var action = $(this).attr('href');

//			jConfirm('Are you sure to delete?', 'Delete Premio', function(r) {
//				if (r) {
					$.ajax({
						dataType: 'json', 
						type: "POST", 
...
//			);
	...
	/**
	 *
	 * Validates PromocionPremio form
	 * @param selector
	 */
