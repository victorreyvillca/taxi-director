...
com.em.Venta.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblVenta input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblVenta input[id^=check-slave-]').attr('checked', false);
			}
		});
	}},
...
//				clickToUpdate('#tblProducto a[id^=update-datoPromocion-]');
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			event.preventDefault();
			// Serializes items checked
			var items = $('#tblVenta :checked');
			var itemsChecked = items.serialize();
			if (itemsChecked == '') {
				alert.flashInfo('No hay Venta seleccionado', {header: com.em.Alert.NOTICE});
				return;
			}
			var action = $(this).attr('href');
