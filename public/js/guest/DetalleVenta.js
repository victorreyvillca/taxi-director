...
//		oSettings._iDisplayLength = 3;
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblDetalleVenta input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblDetalleVenta input[id^=check-slave-]').attr('checked', false);
			}
		});		
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"fnDrawCallback": function() {
				clickToUpdate('#tblDetalleVenta a[id^=update-detalleVentaPremio-]');
			},
		...
		columns.push({
			...
			fnRender : function (oObj) {
				if (oObj.aData[3] == 0) {
					...
					return '<input type="text" name="cantidades[]" id="cantidad-'+oObj.aData[0]+'" value="'+oObj.aData[3]+'">';
				}
			}
		});
		columns.push({"sWidth": "12%"});
		columns.push({"sWidth": "16%"});
		columns.push({
			"bSortable": false,
			"sWidth": "2%",
			"sClass": "checkColumn",
...
//		});
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			var items = $('#tblDetalleVenta :checked');
