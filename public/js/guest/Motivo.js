...
com.em.Motivo.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblMotivo input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblMotivo input[id^=check-slave-]').attr('checked', false);
			}
		});
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"oLanguage": {
				"sEmptyTable": 'No existe Motivos.'
			},
			"fnDrawCallback": function() {
				clickToUpdate('#tblMotivo a[id^=edit-motivo-]');
			},
	...
	getColumns: function() {with (this) {
		...
		columns.push({
			...
			fnRender : function (oObj){
				return '<a id="edit-motivo-'+oObj.aData[0]+'" href="'+url.toUpdate+'/id/'+oObj.aData[0]+'">'+oObj.aData[1]+'</a>';
				}
			});
...
//							setValidatorForm("#formId");
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			event.preventDefault();
			// Serializes items checked
			var items = $('#tblMotivo :checked');
			var itemsChecked = items.serialize();
			if (itemsChecked == '') {
				alert.flashInfo('No hay Motivo seleccionado', {header: com.em.Alert.NOTICE});
				return;
			}
			var action = $(this).attr('href');
