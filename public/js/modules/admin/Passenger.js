...
//		oSettings._iDisplayLength = 3;
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblPassenger input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblPassenger input[id^=check-slave-]').attr('checked', false);
			}
		});		
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"fnDrawCallback": function() {
				clickToUpdate('#tblPassenger a[id^=update-passenger-]');
			},
	...
	getColumns: function() {with (this) {
		...
		columns.push({
			...
			fnRender : function (oObj){
				return '<a id="update-passenger-'+oObj.aData[0]+'" href="'+url.toUpdate+'/id/'+oObj.aData[0]+'">'+oObj.aData[1]+'</a>';
				}
			});
...
//							console.log('hello');
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			var items = $('#tblPassenger :checked');
