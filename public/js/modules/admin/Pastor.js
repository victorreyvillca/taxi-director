...
com.em.Pastor.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblPastor input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblPastor input[id^=check-slave-]').attr('checked', false);
			}
		});
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"fnDrawCallback": function() {
				clickToUpdate('#tblPastor a[id^=update-pastor-]');
			},
	...
	getColumns: function() {with (this) {
		...
		columns.push({
			...
			fnRender : function (oObj){
				return '<a id="update-pastor-'+oObj.aData[0]+'" href="'+url.toUpdate+'/id/'+oObj.aData[0]+'">'+oObj.aData[1]+'</a>';
				}
			});
