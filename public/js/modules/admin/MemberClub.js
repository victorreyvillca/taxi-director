...
com.em.MemberClub.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblMemberClub input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblMemberClub input[id^=check-slave-]').attr('checked', false);
			}
		});
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"fnDrawCallback": function() {
				clickToUpdate('#tblMemberClub a[id^=update-memberClub-]');
			},
	...
	getColumns: function() {with (this) {
		...
		columns.push({
			...
			fnRender : function (oObj){
				return '<a id="update-memberClub-'+oObj.aData[0]+'" href="'+url.toUpdate+'/id/'+oObj.aData[0]+'">'+oObj.aData[1]+'</a>';
				}
			});
		columns.push({"sWidth": "15%"});//4
		//columns.push({"sWidth": "13%"});
		columns.push({"sWidth": "15%"});//5
		columns.push({"sWidth": "16%"});//2
		columns.push({"sWidth": "15%"});//1
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			var items = $('#tblMemberClub :checked');
			...
			jConfirm('Are you sure to delete?', 'Delete Member Club', function(r) {
				if (r) {
					$.ajax({
						dataType: 'json', 
						type: "POST", 
